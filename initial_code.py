...
# === Save Final Predictions from the Last Model ===
...
print(auc_results.round(4))

# === Optional: Summary AUCs ===
macro_auc = auc_results.mean().round(4)
print("\nMacro AUC Summary:")
print(macro_auc)

auc_results_rounded = auc_results.round(4)
auc_results_rounded.to_csv("auc_scores_per_label.csv")
with open("macro_auc_summary.txt", "w") as f:
    f.write("Macro AUC Summary (average AUC per model):\n")
    f.write(macro_auc.to_string())
    f.write("\nAUC Scores Per Label:")
    f.write(auc_results.round(4).to_string())
